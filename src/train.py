...
def build_linear_regression_models(data: pd, step: int):
    ...
    X = data.drop("timestamp", axis=1).drop(target_metric, axis=1)
    ...
    if best_score:
        ...
        dump(best_model, f'models/best_model-{step}.joblib')
        dump(list(X_train.columns), f'models/predictor_columns-{step}.joblib')
    else:
...
def run():
    for step in app_settings.get('predict_intervals', []):
        data = pd.read_csv(f"datasets/dataset-{step}.csv")
